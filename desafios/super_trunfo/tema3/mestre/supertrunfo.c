...
int main () {
    ...
    printf("PIB per capita da Carta 1: %.0f reais\n", pibPerCapita);

    // Carta 2
    printf("Carta 2:\nEstado: %s\nCódigo: %s\nNome do Local: %s\n", estado2, codCarta2, local2);
    printf("População: %lu\nÁrea: %.0f km²\nPIB: %.0f\nNúmero de Pontos Turísticos: %d\nDensidade da população: %.2f hab/km²\n", povo2, area2, pib2, numPontosTuristicos2, densidade2);
    printf("PIB per capita da Carta 2: %.0f reais\n\n", pibPerCapita2);
    ...
    printf("Esse é o Super Poder da Carta 1: %.0f\n", superPoder);
    printf("Esse é o Super Poder da Carta 2: %.0f\n", superPoder2);
    ...
    printf("Qual sua primeira escolha? \n");
    scanf("%d", &escolha);
    printf("Qual sua segunda escolha? \n");
    scanf("%d", &escolha2);

    if (escolha == escolha2) {
        printf("Você escolheu o mesmo atributo para comparar, tente novamente!\n");
        return 0;
    }
    ...
    if (escolha == 0) {
        ...
        if (povo > povo2) {
            printf("%s tem a maior população %lu -> Venceu!\n", local, povo);
        } else if (povo2 > povo) {
            printf("%s tem a maior populaçõa %lu -> Venceu!\n", local2, povo2);
        } else {
            printf("%s e %s possuem mesma população, EMPATE!\n", local, local2);
        }
        
    } else if (escolha == 2) {
        printf("Comparando as áreas: Local - 1 %.0f e Local 2 - %.0f\n", area, area2);
        if (area > area2) {
            printf("%s tem maior área %.0f -> Venceu!\n", local, area);
        } else if (area < area2) {
            printf("%s tem maior área %.0f -> Venceu!\n", local2, area2);
        } else {
            printf("%s e %s possuem mesma área, EMPATE!\n", local, local2);
        }

    } else if (escolha == 3) {
        printf("Comparando o PIB: Local 1 - %.0f e Local 2 - %.0f\n", pib, pib2);
        if (pib > pib2) {
            printf("%s tem maior PIB %.0f -> Venceu!\n", local, pib);
        } else if (pib < pib2) {
            printf("%s tem maior PIB %0.f -> Venceu!\n", local2, pib2);
        } else {
            printf("%s e %s possuem mesmo PIB, EMPATE!\n", local, local2);
        }

    } else if (escolha == 4) {
        printf("Comparando o número de pontos turísticos: Local 1 - %d e Local 2 - %d\n", numPontosTuristicos, numPontosTuristicos2);
        if (numPontosTuristicos > numPontosTuristicos2) {
            printf("%s tem maior quantidade de números de pontos turísticos %d -> Venceu!\n", local, numPontosTuristicos);
        } else if (numPontosTuristicos < numPontosTuristicos2) {
            printf("%s tem maior quantidade de números de pontos turísticos %d -> Venceu!\n", local2, numPontosTuristicos2);
        } else {
            printf("%s e %s possuem mesma quantidade de pontos turísticos, EMPATE!\n", local, local2);
        }

    } else if (escolha == 5) {
        printf("Comparando a densidade demográfica: Local 1 - %.2f e Local 2 - %.2f\n", densidade, densidade2);
        if (densidade < densidade2) {
            printf("%s tem menor densidade demográfica %.2f -> Venceu!\n", local, densidade);
        } else if (densidade > densidade2) {
            printf("%s tem menor densidade demográfica %.2f -> Venceu!\n", local2, densidade2);
        } else {
            printf("%s e %s possuem mesma densidade demográfica, EMPATE!\n", local, local2);
        }
    }
    ...
    float somaAtributosCarta1 = povo + area + pib + numPontosTuristicos + densidade + pibPerCapita;    
    float somaAtributosCarta2 = povo2 + area2 + pib2 + numPontosTuristicos2 + densidade2 + pibPerCapita2;
    int exibir;

    printf("Agora veremos a soma dos atributos numéricos das duas cartas.\n");
    printf("A carta com a maior soma vencerá!\n");

    printf("Agora veremos a soma dos atributos numéricos das duas cartas.\n");
    printf("A carta com a maior soma vencerá!\n");


    exibir = somaAtributosCarta1 > somaAtributosCarta2 ? printf("A soma da Carta 1: %.0f e a soma da Carta 2: %.0f -> Carta 1 Venceu!\n", somaAtributosCarta1, somaAtributosCarta2) : printf("A soma da Carta 2: %.0f e a soma da Carta 1: %.0f -> Carta 2 Venceu!\n", somaAtributosCarta2, somaAtributosCarta1);

    exibir = somaAtributosCarta1 == somaAtributosCarta2 ? printf("As duas cartas possuem a mesma soma: %.0f, EMPATE!\n", somaAtributosCarta1) : 0;
